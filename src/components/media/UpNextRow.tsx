...
export function UpNextRow({ 
  ...
  return (
    <div className="mb-8">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">Up Next</h2>

      <div className="relative group/row">
        {canLeft && (
          <button onClick={() => scroll("left")} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-30 w-12 h-12 flex items-center justify-center bg-black/80 hover:bg-black border border-white/10 hover:border-white/30 rounded-full text-white shadow-2xl backdrop-blur-sm transition-all duration-200 hover:scale-110">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
          </button>
        )}

        <div ref={scrollRef} className="flex overflow-x-auto scrollbar-hide pb-4 pt-2 -mt-2 -mx-2 px-2" style={{ gap: "16px", scrollSnapType: "x mandatory" }}>
          {loading ? (
            [...Array(5)].map((_, i) => (
            ...
            items.map((ep) => {
              ...
              const epImgSrc = hasEpImage ? `${authData.serverUrl}/Items/${ep.Id}/Images/Primary?fillWidth=600&quality=90&api_key=${authData.token}` : `${authData.serverUrl}/Items/${ep.SeriesId}/Images/Backdrop?fillWidth=600&quality=90&api_key=${authData.token}`;

              return (
                <div key={ep.Id} className="flex-shrink-0 w-[280px] group cursor-pointer" style={{ scrollSnapAlign: "start" }} onClick={() => openDetails(ep)}>
                  <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-[#1e1e1e] mb-3 shadow-lg group-hover:scale-[1.04] transition-all duration-300">
                    <img src={epImgSrc} alt={ep.Name} className="w-full h-full object-cover transition duration-300 group-hover:brightness-75" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = SAFE_PLACEHOLDER; }} />
        ...
        {canRight && !loading && (
          <button onClick={() => scroll("right")} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-30 w-12 h-12 flex items-center justify-center bg-black/80 hover:bg-black border border-white/10 hover:border-white/30 rounded-full text-white shadow-2xl backdrop-blur-sm transition-all duration-200 hover:scale-110">
