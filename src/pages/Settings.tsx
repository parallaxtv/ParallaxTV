...
import { HeroBannerSettings } from "../components/settings/HeroBannerSettings";
...
function IconImage()     { return <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" strokeLinejoin="round" d="M21 15l-5-5L5 21"/></svg>; }
...
const NAV_ITEMS = [
  ...
  { id: "hero",       label: "Hero Banner",       Icon: IconImage    },
...
export function Settings({ authData, onLogout }: { authData: AuthData; onLogout: () => void }) {
  ...
  function renderSection() {
    switch (activeSection) {
      ...
      case "hero":       return <HeroBannerSettings authData={authData} />;
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] text-white flex animate-[fadeIn_0.2s_ease-out]">
      ...
      <aside className="w-60 flex-shrink-0 border-r border-white/6 flex flex-col">
          ...
          {NAV_ITEMS.map(({ id, label, Icon }) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <span className="ml-auto w-1 h-4 rounded-full bg-[var(--color-accent)] shadow-[0_0_8px_var(--color-accent-glow)]" />
                )}
              </button>
            );
          })}
        ...
        <div className="px-5 py-5 border-t border-white/5">
          ...
          <p className="text-[10px] text-gray-800 mt-0.5">v0.1.1</p>
        </div>
      </aside>
