...
type SortOption = "SortName" | "DateCreated" | "PremiereDate" | "CommunityRating";
  ...
  return (
    <div className="cursor-pointer group" onClick={() => onSelect(item)}>
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-white/5 mb-3 shadow-lg transition-all duration-300 group-hover:scale-[1.03] group-hover:shadow-xl">
        ...
        <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-black/60 backdrop-blur-md border border-white/10 px-1.5 py-0.5 rounded-md z-10">
          <svg className="w-3 h-3 fill-[var(--color-accent)]" viewBox="0 0 24 24">
...
export function Favorites({ authData, onLogout }: { authData: AuthData; onLogout: () => void }) {
    ...
    async function load() {
      try {
        ...
        const res = await itemsApi.getItems({
          ...
          sortBy: [sortBy],
          sortOrder: [sortBy === "SortName" ? SortOrder.Ascending : SortOrder.Descending],
      ...
      <main className="flex-1 flex flex-col relative h-screen overflow-y-auto scrollbar-hide">
        ...
        <div className="px-12 pt-14 pb-5 flex flex-wrap items-center gap-4 border-b border-white/5
          ...
          <div className="flex items-center gap-3 mr-2">
            <h1 className="text-lg font-black text-white/90 tracking-wide drop-shadow-sm">Favorites</h1>
        ...
        <div className="px-12 py-8 pb-24">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-white/60 text-lg font-bold mb-2 tracking-wide">No favorites yet</p>
              <p className="text-white/40 text-sm font-medium">Tap the heart on any title to add it here.</p>
            </div>
          ) : (
            <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))" }}>
              {items.map((item) => (
